...
StressResult = dict[str, object]


def make_log(node_id: str, final_action: Action) -> ExecutionLog:
    log = ExecutionLog()
...
def build_valid_stress_logs() -> list[tuple[str, ExecutionLog]]:
    ...
    for index in range(1, 11):
        ...
        logs.append((node_id, make_log(node_id, final_action)))
    return logs
...
def build_invalid_stress_cases() -> list[tuple[str, str, list[ExecutionEvent]]]:
    ...
    return [
        ...
        (
            "duplicate_causal_id",
            base,
            [valid_start, ExecutionEvent.create(base, Action.COMPLETE, 1)],
        ),
        ...
        (
            "terminal_reentry",
            base,
            [valid_start, valid_complete, ExecutionEvent.create(base, Action.START, 3)],
        ),
        (
            "terminal_failover",
            base,
            [
                valid_start,
                ExecutionEvent.create(base, Action.FAIL, 2),
                ExecutionEvent.create(base, Action.COMPLETE, 3),
            ],
        ),
    ]


def valid_result(name: str, state: NodeState) -> StressResult:
    return {
        "case": name,
        "valid": True,
...
def invalid_result(name: str, halted: bool, reason: str) -> StressResult:
    return {
        "case": name,
        "valid": False,
        "halted": halted,
...
def run_stress_validation() -> list[StressResult]:
    results: list[StressResult] = []

    for node_id, log in build_valid_stress_logs():
        state = replay_events(log.entries(), node_id)
        results.append(valid_result(node_id, state))

    for name, node_id, events in build_invalid_stress_cases():
        try:
            replay_events(events, node_id)
        except RuntimeErrorReason as exc:
            results.append(invalid_result(name, True, str(exc)))
        else:
            results.append(invalid_result(name, False, "invalid execution was accepted"))

    results.append(run_replay_mismatch_stress_case())
    return results


def run_replay_mismatch_stress_case() -> StressResult:
    node_id = "stress-replay-mismatch-node"
    log = make_log(node_id, Action.COMPLETE)
    ...
    try:
        ...
        return invalid_result("replay_mismatch", True, str(exc))
    return invalid_result("replay_mismatch", False, "replay mismatch was accepted")
